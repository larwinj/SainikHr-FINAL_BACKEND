...
async function subscription(req, res) {
  try {
    ...
    // Calculate video snapshot expiry (7 days) for Premium plans
    let profileVideoRequestExpiry = null;
    if (planName.includes("premium") && plan.profileVideo) {
      profileVideoRequestExpiry = new Date(subscribedAt);
      profileVideoRequestExpiry.setDate(
        profileVideoRequestExpiry.getDate() + 7
      );
    }
    ...
    // Generate new JWT token with streamlined payload
...
async function deletePostedJob(req, res) {
  try {
    ...
    if (req.user?.role === "admin" && req.user?.manageJobs) {
      ...
      return res
        .status(200)
        .json({ message: "The Posted Job is deleted Successfully" });
    }
    ...
    if (existingJob.userId !== userId) {
      ...
      return res
        .status(403)
        .json({ message: "You don't have permission to delete this job post" });
    }

    await Job.destroy({ where: { jobId } });

    const existingUser = await User.findOne({ where: { userId } });
    const updatedPostedJobs = (existingUser?.postedJobs || []).filter(
      (id) => id !== jobId
    );
...
async function getJobs(req, res) {
  try {
    ...
    if (jobIds.length === 1) {
      ...
      await Job.update({ data: jobData }, { where: { jobId } });
      ...
      const formattedJob = {
        ...
        planData: {
          ...
          expireAt:
            job.User?.Access && job.User?.Access?.CorporatePlan
              ? new Date(
                  new Date().getTime() +
                    (job.User.Access.CorporatePlan.durationUnit === "days"
                      ? job.User.Access.CorporatePlan.durationValue *
                        24 *
                        60 *
                        60 *
                        1000
    ...
    const whereClause = jobIds.length > 1 ? { jobId: { [Op.in]: jobIds } } : {};
...
async function updateProfile(req, res) {
  try {
    ...
    const existingUser = await User.findOne({ where: { userId } });
    const existingCorporateDetails = await CorporateDetails.findOne({
      where: { userId },
    });
    ...
    await sequelize.transaction(async (t) => {
      ...
      if (existingCorporateDetails) {
        ...
        await CorporateDetails.create(corporateDetailsUpdateFields, {
          transaction: t,
        });
      }
    });
  ...
  try {
    ...
    if (!corporateId || !jobId || !userId || !resumeId) {
      return res
        .status(400)
        .json({
          message: "Corporate ID, Job ID, Resume ID, and User ID are required!",
        });
    }

    const [existingCorporate, existingUser, existingJob, existingResume] =
      await Promise.all([
    ...
    if (
      ...
      return res
        .status(404)
        .json({ message: "Corporate or User or Job or Resume not found!" });
    }
    ...
    if (application) {
      if (!application.corporateMatched) {
        ...
        const twentyEightDaysLater = new Date(
    ...
    return res
      .status(200)
      .json({ message: "Corporate matched the user's profile for the job!" });
  } catch (error) {
...
async function requestProfileVideo(req, res) {
  try {
    ...
    if (!subscribedPlan) {
      return res
        .status(404)
        .json({ message: "Subscribed plan not found for user" });
    }
  ...
  try {
    ...
    if (skillsMatch) {
      try {
        ...
        return res
          .status(400)
          .json({ error: "Invalid skillsMatch format, should be an array." });
      }
    }
    ...
    for (const resume of resumes) {
      ...
      if (skillsArray.length > 0) {
        const lowerResumeSkills = resumeSkills.map((skill) =>
          skill.toLowerCase()
        );
...
async function recordJobApplication(req, res) {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Application recorded successfully" });
  } catch (error) {
